...
namespace GitAspx.Controllers {
	...
	public class RpcController : BaseController {
		...
		[HttpPost]
		public ActionResult UploadPack(string store, string project) {
			return ExecuteRpc(store, project, "upload-pack", repository => {
				repository.Upload(GetInputStream(), Response.OutputStream);
			});
		}

		[HttpPost]
		public ActionResult ReceivePack(string store, string project) {
            return ExecuteRpc(store, project, "receive-pack", repository =>
            {
		...
		ActionResult ExecuteRpc(string store, string project, string rpc, Action<Repository> action) {
			...
			var repository = repositories.GetRepository(store, project);
