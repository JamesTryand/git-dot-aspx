...
namespace GitAspx.Controllers {
	...
	public class InfoRefsController : BaseController {
		...
		public ActionResult Execute(string store, string project, string service) {
			...
			if(isUsingSmartProtocol) {
				return SmartInfoRefs(store, service, project);
			}

			// working with the dumb protocol.
			return DumbInfoRefs(store, project);
		}

		ActionResult SmartInfoRefs(string store, string service, string project) {
		...
		ActionResult DumbInfoRefs(string store, string project) {
			Response.WriteNoCache();
